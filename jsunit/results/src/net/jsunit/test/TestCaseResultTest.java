 ...
 * ***** BEGIN LICENSE BLOCK *****
   ...
   @author Edward Hieatt
 */
public class TestCaseResultTest extends JsUnitTest {
	public TestCaseResultTest(String name) {
		super(name);
	}
	public void testBuildSuccessfulResultFromString() {
		TestCaseResult result = TestCaseResult.fromString("testFoo|1.3|S||");
	...
	public void testBuildErrorResultFromString() {
		TestCaseResult result = TestCaseResult.fromString("testFoo|1.3|E|Error Message|");
		assertEquals("testFoo", result.getName());
		assertEquals(1.3d, result.getTime(), .001d);
	...
	public void testBuildFailureResultFromString() {
		TestCaseResult result = TestCaseResult.fromString("testFoo|1.3|F|Failure Message|");
		assertEquals("testFoo", result.getName());
		assertEquals(1.3d, result.getTime(), .001d);
		assertFalse(result.hadError());
	...
	public void testBuildFromXmlFragment() {
		...
		assertEquals("testFoo", reconstitutedResult.getName());
		...
		assertEquals("Failure Message", reconstitutedResult.getFailure());
	}
}
